...
public abstract class Page implements Cloneable
{
    ...
    static Page createEmptyLeaf(MVMap<?, ?> map, boolean capable) {
        ...
        return createLeaf(map, 0, keys, values, PAGE_LEAF_MEMORY);
    }
     ...
     * @return the page
     */
    public static Page createNode(MVMap<?, ?> map, int keyCount,
            Object keys, PageReference[] children,
            long totalCount, int memory) {
        assert keys != null;
        Page page = new NonLeaf(map, keyCount, keys, children, totalCount);
     ...
     * @return the page
     */
    public static Page createLeaf(MVMap<?, ?> map, int keyCount, Object keys, Object values, int memory) {
        assert keys != null;
        Page page = new Leaf(map, keyCount, keys, values);
        page.initMemoryAccount(memory);
        return page;
    }
    ...
    /**
     * Split the current keys storage into two.
     *
     * @param aCount size of the first storage.
     * @param bCount size of the second storage.
     * @return the second storage.
     */
    ...
    /**
     * Insert a key into the key storage
     *
...
//                    "Memory calculation error " + memory + " != " + calculateMemory();
    ...
    private static class NonLeaf extends Page
    {
        ...
        */
        ...
        public Page split(int at) {
            ...
            Page newPage = createNode(map, b - 1, bKeys, bChildren, t, 0);
            if(isPersistent()) {
                recalculateMemory();
            }
            return newPage;
    ...
    private static class Leaf extends Page
    {
        ...
        public Page split(int at) {
            ...
            Page newPage = createLeaf(map, b, bKeys, bValues, 0);
            if(isPersistent()) {
                recalculateMemory();
            }
            return newPage;
        ...
        public void trimLeaf() {
            ...
            if(values != null) {
                ExtendedDataType valueType = map.getExtendedValueType();
                if (isPersistent()) {
                    addMemory(-valueType.getMemory(valueType.getValue(values, getKeyCount())));
                }
            }
        }
